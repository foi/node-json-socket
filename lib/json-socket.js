...
JsonSocket.sendReceive = function(options) {
    return new Promise(function(resolve, reject) {
        if (!options.message) throw Error('Message is not specified!')
        var socket = new JsonSocket(new net.Socket(), {delimeter: options.delimeter || "#"})
        if (options.timeout) socket.setTimeout(options.timeout)
        if (options.host && options.port) {
            socket.connect(options.port, options.host)
        }
        else if (options.unixSocket) {
            socket.connect(options.unixSocket)
        }
        else {
            throw Error('Remote address is not specified!')
        }
        socket.on('connect', function() {
            socket.sendMessage(options.message, function(err) {
                if (err) {
                    socket.end()
                    reject(err)
                }
